...
当前服务采用“控制面走 HTTP、数据面走共享文件系统”的方式：调用方把不可变的 Document Package 目录打包为 `.hepkg.tar.gz`，通过 `POST /v1/runs` 以 multipart 上传给 HE。API 不接收 `file://` URI、原始文件流、模型地址或模型密钥。外部调用方不挂载 `/exchange` 共享卷，只通过 HTTP 上传归档、轮询状态、下载结果。
...
| 共享存储 | 不需要；调用方通过 HTTP 上传 `.hepkg.tar.gz` 并下载结果 | API 与 Worker 共享同一 `/exchange` 卷；外部调用方不挂载 |
...
!!! warning "传输封装与内容契约分离"
    ...
    Document Package v1.1 目录是**内容契约**；`.tar.gz` 只是跨服务传输封装。两个哈希必须分别校验，不能用 tarball 字节流哈希代替 Package 内部完整性校验：

    - `package_fingerprint`：解压后 Document Package 的规范 SHA-256（由 `document_package_fingerprint` 计算）。
    - `transport_sha256`：上传 `.tar.gz` 字节流的 SHA-256。

    归档根目录必须直接包含 `manifest.json`、`outline.json`、`provenance.jsonl`、`extraction-brief.yaml` 和 `content/`，不得额外嵌套顶层目录，否则返回 `PACKAGE_ARCHIVE_INVALID`。
...
- `GET /v1/capabilities` 包含 `course_graph`、Document Package `1.1`、`http-multipart` 和 timeline `1.0`。
...
- 调用方能通过 `GET /v1/runs/{run_id}/result` 下载 `course-graph.json` 并校验 ETag/SHA-256。
...
sequenceDiagram
    ...
    P->>P: 构建 Document Package 目录，打包为 .hepkg.tar.gz
    P->>P: 计算规范包指纹 package_fingerprint 与 transport_sha256
    P->>A: POST /v1/runs (multipart: package + 双哈希 + options) + Idempotency-Key
    A->>A: 流式落盘并校验 transport_sha256
    A->>A: 安全解包到 staging，校验 Schema/布局/逐文件哈希/规范指纹
    A->>A: 原子发布 pkg_<fingerprint>.hepkg，预留 runs/<run_id>/
    A-->>P: 202 status=queued + run_id + HTTP links
    W->>A: 领取任务，读 pkg_<fingerprint>.hepkg，执行 Pipeline
    W->>W: 原子写 state/progress.json（动态 activity/message）
    loop 直到终态
        P->>A: GET /v1/runs/{run_id}
        A->>A: 合并 DB 生命周期 + 校验 progress.json owner
        A-->>P: queued/running/completed/failed/cancelled + 动态进度
    end
    W->>W: 原子发布 artifacts/，最后写 _SUCCESS
    W->>A: 标记 completed
    P->>A: GET /v1/runs/{run_id}/result
    A->>A: 校验 DB 终态 + _SUCCESS + manifest 哈希 + 文件 SHA-256
    A-->>P: 200 流式返回 course-graph.json (ETag/Content-Length)
    C->>C: 校验 ETag/SHA-256，原子保存后标记 COMPLETED
```
...
```mermaid
flowchart TD
    A["构建并打包 .hepkg.tar.gz，计算双哈希"] --> C["POST /v1/runs (multipart)，携带幂等键"]
    C -->|"202"| D["保存 run_id 并轮询 GET /v1/runs/{run_id}"]
    C -->|"超时/连接断开/5xx"| C1["使用相同幂等键和完全相同归档重试"]
    C1 --> C
    C -->|"400 PACKAGE_REQUIRED / INVALID_MULTIPART"| C2["补全 multipart 字段或修正 options JSON"]
    C -->|"413 PACKAGE_UPLOAD_TOO_LARGE"| C3["减小归档体积后重新打包"]
    C -->|"422 PACKAGE_ARCHIVE_INVALID / EXPANDED_TOO_LARGE"| C4["修复归档结构/成员，重新打包"]
    C -->|"422 PACKAGE_TRANSPORT_HASH_MISMATCH"| C5["重新计算 transport_sha256"]
    C -->|"422 DOCUMENT_PACKAGE_HASH_MISMATCH"| C6["核对规范指纹算法，重新打包"]
    C -->|"409 IDEMPOTENCY_KEY_CONFLICT"| C7["停止自动重试，核对逻辑任务与请求"]
    D -->|"queued/running"| D
    D -->|"completed"| E["GET /v1/runs/{run_id}/result 下载 course-graph.json"]
    D -->|"failed + resumable=true"| F["POST resume"]
    F --> D
    D -->|"failed + resumable=false"| G["人工处理或修复后创建新任务"]
    D -->|"需要取消"| H["POST cancel，继续轮询终态"]
    H --> D
    E -->|"200 + ETag/SHA-256 校验通过"| I["原子保存结果，标记 COMPLETED"]
    E -->|"404 RUN_NOT_FOUND"| E1["核对 run_id"]
    E -->|"409 ARTIFACTS_NOT_READY"| D
    E -->|"500 ARTIFACT_STATE_INCONSISTENT"| J["停止导入并告警，禁止消费半成品"]
```

## 3. `.hepkg.tar.gz` 传输封装契约

### 3.1 归档结构

调用方把已通过字节校验的 Document Package 目录打包为 gzip tar 归档，归档根目录必须直接包含 Package 文件：

```text
course.hepkg.tar.gz
└── (解压根)
    ├── manifest.json
    ├── extraction-brief.yaml      # v1.1 必需
    ├── outline.json
    ├── provenance.jsonl
    └── content/
        ├── chapter-01.md
...
- 归档根下嵌套了一个不确定名称的顶层目录（例如 `course/course.hepkg/manifest.json`）。
- 包含绝对路径、`..`、软链接、硬链接、设备文件或 FIFO。
- gzip/tar 损坏，或解压后大小/成员数超过服务端限制。

### 3.2 双哈希校验

| 字段 | 计算对象 | 校验时机 |
|---|---|---|
| `package_fingerprint` | 解压后 Document Package 的规范 JSON 指纹 | 解包 + Schema 校验后 |
| `transport_sha256` | 上传 `.tar.gz` 字节流 | 流式落盘后立即校验 |

两者不一致分别返回 `DOCUMENT_PACKAGE_HASH_MISMATCH` 与 `PACKAGE_TRANSPORT_HASH_MISMATCH`。

压缩包上传上限由 `HE_SERVICE_MAX_UPLOAD_BYTES` 配置，默认
`500000000` 字节（500 MB）。API 在流式读取请求时增量执行限制，超过上限后立即停止，
该数值不在路由中写死；解压大小、成员数和进度刷新周期使用独立配置。

HE Worker 通过 `HE_SERVICE_PIPELINE_MAX_WORKERS` 配置单个任务同时处理的文档块数量，
默认值为 `2`，且必须是正整数。它属于部署侧设置，调用方不能通过 `POST /v1/runs`
覆盖。生成模型与 Embedding 请求的实际并发还分别受 Model Profile 中
`recommended_concurrency` 的限制，最终采用两者中的较小值。

### 3.3 安全解包

服务端在 Python 3.11/3.12 上显式校验每个 tar 成员，不依赖版本相关的 `extractall()` 默认安全行为。任何失败都清理上传临时文件和 staging 目录，不影响既有正式 Package。归档上传成功并原子发布为 `pkg_<fingerprint>.hepkg/` 后，临时 `.upload-*.tar.gz` 被删除。
...
| `POST` | `/v1/runs` | 202 | multipart 上传归档并创建或幂等获取任务 |
| `GET` | `/v1/runs/{run_id}` | 200 | 查询任务（合并 DB 生命周期与文件进度） |
| `POST` | `/v1/runs/{run_id}/cancel` | 200 | 请求取消 |
| `POST` | `/v1/runs/{run_id}/resume` | 202 | 恢复可恢复失败任务 |
| `GET` | `/v1/runs/{run_id}/result` | 200 | 流式下载 `course-graph.json` 主结果 |
| `GET` | `/v1/runs/{run_id}/result-metadata` | 200 | 获取经过校验的脱敏结果元数据 |
| `GET` | `/v1/runs/{run_id}/artifacts` | 200 | 获取成功任务的产物清单 |
| `GET` | `/v1/runs/{run_id}/errors` | 200 | 获取脱敏失败历史 |
...
调用方可在启动或定期健康检查时验证所需能力。`package_schemes` 描述的是跨服务传输方式，不代表 Worker 内部存储路径；`http-multipart` 表示调用方必须上传 `.hepkg.tar.gz`。`timeline_schema_versions` 用于在启用严格时间线校验前确认服务端支持固定时间线 v1。`contracts` 给出可机器读取的 Run status 和脱敏结果元数据 JSON Schema。当前响应不列出可用的 Model Profile 名称；该名称需由部署双方另行约定。
...
在排队前验证已发布的内容寻址 Package、所有声明文件及规范包指纹。此接口只读，不创建任务。调用方需先通过 `POST /v1/runs` 上传并发布 Package（或由运维预先发布），再以返回的 `package_fingerprint` 调用本接口。
...
```json
{
  ...
  "package_fingerprint": "<64 位小写十六进制规范包指纹>"
}
```
...
| `package_fingerprint` | string | 是 | `^[0-9a-f]{64}$`，需先通过 `POST /v1/runs` 上传发布或由运维预先发布 |
...
|---:|---|---|---|
| 422 | `DOCUMENT_PACKAGE_INVALID` | 结构、Schema、文件或内容完整性错误，或 fingerprint 未对应已发布 Package | 修复后重新上传发布，再验证 |
...
创建异步任务。请求为 `multipart/form-data`，必须带请求头：
...
该接口不接受 `system_prompt`、`prompt`、`extraction_brief`、外部 `brief_uri` 或 `package_uri` 字段。系统提示词只能通过归档内 Package 的 Brief 提供。
...
| `options` | JSON string | 否 | 含 `pipeline`/`execution`/`client_context`；缺省时使用课程图默认配置 |
...
```json
{
  ...
  "client_context": {"service": "course-platform", "task_id": "task-789", "course_id": "course-2026-001"}
}
```
...
curl -X POST http://he-api:8000/v1/runs \
  ...
  -F 'options={"pipeline":{"name":"course_graph","profile":{"name":"course_knowledge_graph","version":"1"}},"execution":{"model_profile":"openai-compatible-default"}}'
```

`202 Accepted` 只在以下条件全部满足后返回：上传字节流完整落盘、`transport_sha256` 校验通过、tar 成员通过安全检查、Package 已安全解压、Document Package v1.1 Schema/布局/Brief/逐文件哈希/规范指纹校验通过、staging 已原子发布为 `pkg_<fingerprint>.hepkg`、PostgreSQL 中存在可领取的 `queued` run、`work`/`state`/`diagnostics/attempts` 目录已准备完成。`202` 响应只返回稳定 HTTP links，不返回 `file://` URI。

修改 Brief 后，调用方必须重新计算 Brief 文件 SHA-256、更新 manifest、重新计算规范包指纹、重新打包归档，并在创建新逻辑任务时使用新的 `Idempotency-Key`。使用旧幂等键提交新 Brief 会返回 `409 IDEMPOTENCY_KEY_CONFLICT`。
...
```json
{
  ...
  "links": {
    ...
    "result": "/v1/runs/run_7c73af29b37f40c59f2eea2dfef3d6ad/result",
    "result_metadata": "/v1/runs/run_7c73af29b37f40c59f2eea2dfef3d6ad/result-metadata",
...
响应不再包含 `output` 的 `file://` URI；使用 `links.result` 与 `links.artifacts` 替代。相同 `Idempotency-Key` + 相同有效请求会返回同一个 `run_id`，响应仍为 `202`。幂等比较包含 Package 规范指纹、contract version、pipeline、Profile、Model Profile 指纹及调用方上下文，不包含临时路径或 tar metadata。
...
| 400 | `INVALID_MULTIPART_REQUEST` | multipart 或 `options` 无法解析 |
| 409 | `IDEMPOTENCY_KEY_CONFLICT` | 同一幂等键已用于不同请求 |
...
| 422 | `MODEL_PROFILE_INVALID` | 服务端不存在该 Profile |
| 500 | `PACKAGE_PUBLICATION_FAILED` | staging 无法可靠发布 |
...
运行状态由两个有界、结构化文件组成，而不是从 stdout 或原始日志中解析：

```text
/exchange/runs/<run_id>/state/
  progress.json  # 高频当前快照；ticker 可轮换安全文案
  timeline.json  # 低频固定九阶段摘要；只在开始、完成、失败、跳过、恢复时更新
```

`GET /v1/runs/{run_id}` 始终返回 `Cache-Control: no-store`。API 先读取固定九项 `timeline.json`，再把通过 `run_id`、`attempt` 和当前租约 owner 校验的 `progress.json` 覆盖到当前 running 项，最后从同一份合并结果生成顶层 `activity/message/message_seq/progress`，避免两套状态漂移。快照无效时安全降级，不返回 `5xx`。
...
```json
{
  ...
  "message_seq": 37,
  "progress": {"current": 8, "total": 28, "percent": 28.57},
  "timeline_schema_version": "1.0",
  "timeline": [
    {"activity":"DOCUMENT_INGESTING","label":"读取文档内容","status":"completed","message":"文档内容读取完成","message_seq":2,"progress":null,"started_at":"2026-07-15T06:29:40Z","completed_at":"2026-07-15T06:29:43Z","attempt":1},
    {"activity":"CHUNK_PLANNING","label":"规划处理单元","status":"completed","message":"文档处理单元规划完成","message_seq":2,"progress":{"current":28,"total":28,"percent":100.0},"started_at":"2026-07-15T06:29:43Z","completed_at":"2026-07-15T06:29:45Z","attempt":1},
    {"activity":"EXTRACTING_CHUNK","label":"抽取知识点","status":"running","message":"正在分析第 8/28 个内容块","message_seq":37,"progress":{"current":8,"total":28,"percent":28.57},"started_at":"2026-07-15T06:29:45Z","completed_at":null,"attempt":1},
    {"activity":"DEDUPLICATING","label":"消除重复知识点","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null},
    {"activity":"BUILDING_GLOBAL_EDGES","label":"建立跨章节关系","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null},
    {"activity":"QUALITY_CHECKING","label":"检查图谱质量","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null},
    {"activity":"BUILDING_COMMUNITIES","label":"组织知识主题","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null},
    {"activity":"FINALIZING","label":"整理最终图谱","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null},
    {"activity":"ARTIFACT_PUBLISHING","label":"发布图谱产物","status":"pending","message":"","message_seq":0,"progress":null,"started_at":null,"completed_at":null,"attempt":null}
  ],
  "updated_at": "2026-07-15T06:30:08Z"
}
```
...
每个 activity 在 `timeline` 中恰好出现一次，状态只能是 `pending/running/completed/failed/skipped`，同一时刻最多一个 `running`。内容块进度只更新 `EXTRACTING_CHUNK` 项，不增加数组长度。长模型调用期间 ticker 只更新 `progress.json` 的安全文案和 `message_seq`，不伪造 `current`/`total`。恢复时已完成项不倒退，首次 `started_at` 保留，当前项使用新的 attempt。`timeline.json` 缺失、半写、未知版本或 owner 不匹配时，API 根据 PostgreSQL 生命周期和 `stage` 生成固定九项安全降级结果；`stage=publish` 必须降级为 `ARTIFACT_PUBLISHING`。原始 `.he-run/events.jsonl` 和容器日志永远不进入公共 status。

调用方应把 `status` 作为任务生命周期主状态，把固定 `timeline` 用于 UI，把顶层 `activity/message/message_seq/progress` 作为当前项快捷字段。顶层值始终与当前 running 或 failed TimelineStep 一致。

客户端应把 `status` 作为任务生命周期主状态，把 `stage`、`stage_status`、`activity`、`message` 和 `progress` 用于展示与诊断。
...
数据库内部 `stage` 不是公共时间线枚举；调用方不得据此自行拼时间线。公共 timeline v1 是封闭、固定顺序的九项枚举，如未来调整必须发布新的 `timeline_schema_version`。`attempt` 初始为 `1`，每次成功接受恢复请求加 `1`。
...
### 6.12 `GET /v1/runs/{run_id}/result`

仅在任务和 Artifact 发布都完成后，以文件流返回固定的 `course-graph.json`。该内部接口保留文件传输语义：`Content-Type: application/json`、`Content-Length`、附件文件名和基于文件 SHA-256 的 `ETag`。调用方必须流式接收并校验 ETag，不能把 HE 的附件头或内部路径直接透传给自己的公共 API。

未完成返回 `409 ARTIFACTS_NOT_READY`；任务不存在返回 `404 RUN_NOT_FOUND`；数据库已完成但 `_SUCCESS`、manifest、大小或哈希不一致时返回 `500 ARTIFACT_STATE_INCONSISTENT`。接口不接受文件名或路径参数。

### 6.13 `GET /v1/runs/{run_id}/result-metadata`

返回与已验证结果配套的脱敏 JSON 元数据，不返回文件路径、模型请求、Token/费用、Prompt、日志或 Worker 信息：

```json
{
  "schema_name": "HyperExtractResultMetadata",
  "schema_version": "1.0",
  "run_id": "run_7c73af29b37f40c59f2eea2dfef3d6ad",
  "completed_at": "2026-07-15T06:35:43Z",
  "profile": {"name":"course-knowledge-default","version":"1.1.0","content_hash":"<sha256>","prompt_hash":"<sha256>"},
  "extraction_brief": {"id":"course-knowledge-v1","version":"1.0","content_hash":"<sha256>"},
  "artifact": {"media_type":"application/json","schema_name":"HyperExtractCourseGraph","size_bytes":48210,"sha256":"<course-graph.json sha256>"},
  "performance": {"elapsed_seconds":92.4,"chunk_count":28},
  "quality": {
    "outline_sections":12,
    "extractable_sections":10,
    "covered_sections":9,
    "directly_covered_sections":8,
    "hierarchically_covered_sections":9,
    "outline_coverage":0.9,
    "uncovered_section_ids":["section-10"],
    "knowledge_points":126,
    "relations":84,
    "relation_distribution":{"prerequisite":20,"derivative":18,"related":39,"confusable":7},
...
调用方应交叉校验 `run_id`、`artifact.size_bytes/sha256/schema_name/media_type` 与 result 文件，并核对 Profile 版本和图谱统计。`performance` 当前只公开墙钟耗时与块数，不把“启用 resume 模式”误报为实际发生恢复。

### 6.14 状态与结果元数据 JSON Schema

以下接口提供当前实现生成的 Pydantic JSON Schema，供客户端生成校验器或在 CI 中锁定契约：
...
Run status Schema 将 `timeline_schema_version` 固定为 `1.0`，并把 `timeline` 限制为恰好九项；result metadata Schema 固定 `schema_name/schema_version` 和 Artifact 类型。客户端仍应先读取 capabilities 再启用严格模式。
...
PACKAGE_DIR=/path/to/course-20260714.hepkg   # Document Package 目录
TARBALL=/tmp/course.hepkg.tar.gz

# 打包归档（根目录直接包含 manifest.json 等）
tar -C "$PACKAGE_DIR" -czf "$TARBALL" .

# 计算双哈希
TRANSPORT_SHA256="$(shasum -a 256 "$TARBALL" | awk '{print $1}')"
PACKAGE_FINGERPRINT="$(uv run python -c \
  'import sys, tarfile, tempfile, shutil
from hyperextract.documents import document_package_fingerprint
tmp=tempfile.mkdtemp()
try:
  with tarfile.open(sys.argv[1],"r:gz") as t: t.extractall(tmp)
  print(document_package_fingerprint(tmp))
finally:
  shutil.rmtree(tmp, ignore_errors=True)' "$TARBALL")"
...
curl --fail-with-body \
  ...
  -F 'options={"pipeline":{"name":"course_graph","profile":{"name":"course_knowledge_graph","version":"1"}},"execution":{"model_profile":"openai-compatible-default"},"client_context":{"service":"course-platform","task_id":"task-789","course_id":"course-2026-001"}}'
```
...
# 下载主结果（流式，含 Content-Length / ETag / Content-Disposition）
curl --fail-with-body "$HE_BASE_URL/v1/runs/$RUN_ID/result" -o course-graph.json
curl --fail-with-body "$HE_BASE_URL/v1/runs/$RUN_ID/artifacts"
```

## 10. 当前 v1 边界与兼容性

- 调用方通过 HTTP multipart 上传 `.hepkg.tar.gz`，不再支持 `file://` URI 或共享卷挂载；不支持 S3/OSS URI 或单文件 ZIP。
